...
class TraditionalFeatureExtractor:
    ...
    def extract_mfcc(audio: np.ndarray, sr: int = SAMPLING_RATE, n_mfcc: int = 13):
        mfcc = np.mean(
    ...
    def extract_features(
        ...
        if "mel_spectrogram" in required_features:
            mel_spectrogram = TraditionalFeatureExtractor.extract_melspectrogram(audio, sr)
            result = np.hstack([result, mel_spectrogram])
    
        if "chroma_stft" in required_features:
            chroma_stft = TraditionalFeatureExtractor.extract_chroma_stft(audio, sr)
